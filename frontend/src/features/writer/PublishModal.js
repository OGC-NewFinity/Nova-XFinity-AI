...
const PublishModal = ({ isOpen, onClose, metadata, sections }) => {
  ...
  const steps = [
    ...
    { label: 'Uploading FinityHub Media Assets', icon: 'fa-cloud-arrow-up' },
  ...
  return html`
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl shadow-black/50 overflow-hidden animate-dropdownIn">
        <div className="p-10 text-center">
          ${status === 'publishing' ? html`
            <div className="mb-8">
              ...
              <h3 className="text-2xl font-black text-slate-800 tracking-tight">Syncing to WordPress</h3>
              <p className="text-slate-500 font-medium text-sm mt-2">Connecting your Finity Agent to the WP REST API context.</p>
            </div>
            ...
            <div className="space-y-4 text-left max-w-xs mx-auto">
              ${steps.map((s, i) => html`
                <div key=${i} className=${`flex items-center space-x-4 transition-all duration-500 ${i === step ? 'translate-x-2' : ''} ${i > step ? 'opacity-20' : 'opacity-100'}`}>
                  <div className=${`w-8 h-8 rounded-xl flex items-center justify-center text-xs ${i === step ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'bg-slate-100 text-slate-400'}`}>
                    <i className=${`fa-solid ${i < step ? 'fa-check' : s.icon}`}></i>
                  </div>
                  <span className=${`text-[11px] font-black uppercase tracking-widest ${i === step ? 'text-slate-800' : 'text-slate-400'}`}>${s.label}</span>
                </div>
              `)}
            </div>
          ...
          ` : html`
            <div className="animate-fadeIn">
              ...
              <h3 className="text-3xl font-black text-slate-800 tracking-tight">Article Published!</h3>
              <p className="text-slate-500 font-medium text-sm mt-3 px-6 leading-relaxed">
              ...
              <div className="mt-10 space-y-3">
                <a 
                  href="#" 
                  onClick=${(e) => { e.preventDefault(); alert("Redirecting to mock WordPress Editor..."); }}
                ...
                <button 
                  onClick=${onClose}
                  className="block w-full py-4 bg-white text-slate-500 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-50 transition-all"
                >
        ...
        <div className="bg-slate-50 px-10 py-4 flex items-center justify-between border-t border-slate-100">
