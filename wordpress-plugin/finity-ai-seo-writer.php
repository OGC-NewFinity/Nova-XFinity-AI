...
class Finity_AI_SEO_Writer {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_finity_ai_generate_content', array($this, 'ajax_generate_content'));
        add_action('wp_ajax_finity_ai_test_api_key', array($this, 'ajax_test_api_key'));
        
        // Register AJAX handlers for wizard
        add_action('wp_ajax_finity_ai_wizard_save_step', array($this, 'ajax_wizard_save_step'));
        add_action('wp_ajax_finity_ai_wizard_test_platform', array($this, 'ajax_wizard_test_platform'));
        add_action('wp_ajax_finity_ai_wizard_complete', array($this, 'ajax_wizard_complete'));
        add_action('wp_ajax_finity_ai_wizard_reset', array($this, 'ajax_wizard_reset'));
        
        // Register scheduled event handler for token sync retries
        add_action('finity_ai_retry_token_sync', array($this, 'handle_retry_token_sync'), 10, 4);
    }
    ...
    public function check_activation_redirect() {
        ...
        if (get_transient('finity_ai_activation_redirect')) {
            delete_transient('finity_ai_activation_redirect');
            
            // Only redirect if wizard should be shown
            if ($this->should_show_wizard() && !isset($_GET['page'])) {
                wp_safe_redirect(admin_url('options-general.php?page=finity-ai-wizard'));
                exit;
            }
        }
    }
    ...
    public function add_admin_menu() {
        ...
        if ($this->should_show_wizard()) {
            ...
            add_submenu_page(
                ...
                __('Finity AI Setup Wizard', 'finity-ai'), // Page title
                __('Finity SEO Writer', 'finity-ai'),     // Menu title
                'manage_options',                          // Capability required
                'finity-ai-wizard',                        // Menu slug
            ...
            add_submenu_page(
                'options-general.php',                    // Parent slug (Settings)
                __('Finity SEO Writer', 'finity-ai'),    // Page title
                __('Finity SEO Writer', 'finity-ai'),    // Menu title
                'manage_options',                         // Capability required
                'finity-ai-settings',                     // Menu slug
    ...
    private function should_show_wizard() {
        $settings = get_option('finity_ai_settings', array());
        $wizard_completed = get_option('finity_ai_wizard_completed', false);
    ...
    public function register_settings() {
        ...
        register_setting(
            'finity_ai_settings_group',           // Option group
            'finity_ai_settings',                 // Option name
            array($this, 'sanitize_settings')     // Sanitization callback
        );
        
        // Add settings section
        add_settings_section(
            'finity_ai_api_section',              // Section ID
            __('API Configuration', 'finity-ai'), // Section title
            array($this, 'render_api_section'),   // Callback function
            'finity-ai-settings'                  // Page slug
        );
        
        // Add settings section for content defaults
        add_settings_section(
            'finity_ai_content_section',          // Section ID
            __('Content Defaults', 'finity-ai'),  // Section title
        ...
        add_settings_field(
            ...
            __('OpenAI API Key', 'finity-ai'),    // Field title
        ...
        add_settings_field(
            ...
            __('Gemini API Key', 'finity-ai'),    // Field title
        ...
        add_settings_field(
            ...
            __('Default Tone/Style', 'finity-ai'), // Field title
        ...
        add_settings_field(
            ...
            __('Max Word Count', 'finity-ai'),    // Field title
        ...
        add_settings_section(
            'finity_ai_platform_section',         // Section ID
            __('Platform Sync', 'finity-ai'),    // Section title
        ...
        add_settings_field(
            ...
            __('Platform API Key', 'finity-ai'),  // Field title
        ...
        add_settings_field(
            ...
            __('Platform API URL', 'finity-ai'),  // Field title
        ...
        add_settings_field(
            ...
            __('Platform User ID', 'finity-ai'),  // Field title
    ...
    public function sanitize_settings($input) {
        ...
        if (isset($input['platform_api_url'])) {
            ...
            $sanitized['platform_api_url'] = $url ? $url : 'https://api.finity.ai';
        }
    ...
    public function render_api_section() {
        echo '<p>' . esc_html__('Configure API keys for AI providers. These keys will be used for content generation.', 'finity-ai') . '</p>';
    }
    ...
    public function render_content_section() {
        echo '<p>' . esc_html__('Set default values for content generation. These can be overridden when generating content.', 'finity-ai') . '</p>';
    }
    ...
    public function render_openai_api_key_field() {
        ...
        <input 
            type="password" 
            id="openai_api_key" 
            name="finity_ai_settings[openai_api_key]" 
            value="<?php echo $value; ?>" 
            class="regular-text"
            placeholder="sk-..."
        />
        <button type="button" class="button" id="test-openai-key" style="margin-left: 10px;">
            <?php esc_html_e('Test Key', 'finity-ai'); ?>
        </button>
        <p class="description">
            <?php esc_html_e('Enter your OpenAI API key. Keep this secure and never share it publicly.', 'finity-ai'); ?>
        </p>
        <?php
    }
    ...
    public function render_gemini_api_key_field() {
        ...
        <input 
            type="password" 
            id="gemini_api_key" 
            name="finity_ai_settings[gemini_api_key]" 
            value="<?php echo $value; ?>" 
            class="regular-text"
            placeholder="AIza..."
        />
        ...
        <p class="description">
            <?php esc_html_e('Enter your Google Gemini API key. Keep this secure and never share it publicly.', 'finity-ai'); ?>
        </p>
        <?php
    }
    ...
    public function render_default_tone_field() {
        ...
        $tones = array(
            'professional' => __('Professional', 'finity-ai'),
            'casual' => __('Casual', 'finity-ai'),
            'friendly' => __('Friendly', 'finity-ai'),
            'formal' => __('Formal', 'finity-ai'),
            'conversational' => __('Conversational', 'finity-ai'),
            'technical' => __('Technical', 'finity-ai'),
        );
        ?>
        <select id="default_tone" name="finity_ai_settings[default_tone]" class="regular-text">
        ...
        <p class="description">
            <?php esc_html_e('Default writing tone/style for generated content.', 'finity-ai'); ?>
        </p>
        <?php
    }
    ...
    public function render_max_word_count_field() {
        ...
        <input 
            type="number" 
            id="max_word_count" 
            name="finity_ai_settings[max_word_count]" 
        ...
        />
        <p class="description">
            <?php esc_html_e('Maximum word count for generated articles. Default: 1000 words.', 'finity-ai'); ?>
        </p>
        <?php
    }
    ...
    public function render_platform_section() {
        echo '<p>' . esc_html__('Configure platform sync settings to synchronize token usage with the main platform.', 'finity-ai') . '</p>';
    }
    ...
    public function render_platform_api_key_field() {
        ...
        <input 
            type="password" 
            id="platform_api_key" 
            name="finity_ai_settings[platform_api_key]" 
            value="<?php echo $value; ?>" 
            class="regular-text"
            placeholder="finity_..."
        />
        <p class="description">
            <?php esc_html_e('API key for authenticating with the platform. Get this from your platform account settings.', 'finity-ai'); ?>
        </p>
        <?php
    }
    ...
    public function render_platform_api_url_field() {
        $settings = get_option('finity_ai_settings', array());
        $value = isset($settings['platform_api_url']) ? esc_attr($settings['platform_api_url']) : 'https://api.finity.ai';
        ?>
        <input 
            type="url" 
            id="platform_api_url" 
            name="finity_ai_settings[platform_api_url]" 
            value="<?php echo $value; ?>" 
            class="regular-text"
            placeholder="https://api.finity.ai"
        />
        <p class="description">
            <?php esc_html_e('Base URL of the platform API. Default: https://api.finity.ai', 'finity-ai'); ?>
        </p>
        <?php
    }
    ...
    public function render_platform_user_id_field() {
        ...
        <input 
            type="text" 
            id="platform_user_id" 
            name="finity_ai_settings[platform_user_id]" 
            value="<?php echo $value; ?>" 
            class="regular-text"
            placeholder="user-uuid-here"
        />
        <p class="description">
            <?php esc_html_e('Your user ID on the platform. This is required for token usage synchronization.', 'finity-ai'); ?>
        </p>
        <?php
    }
    ...
    public function render_wizard_page() {
        ...
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'finity-ai'));
        }
        
        // Get current settings if any
        $settings = get_option('finity_ai_settings', array());
        ?>
        <div class="wrap finity-wizard-container">
            <div class="finity-wizard-header">
                <h1><?php esc_html_e('Welcome to Finity AI SEO Writer', 'finity-ai'); ?></h1>
                <p class="description"><?php esc_html_e('Let\'s get you set up in just a few steps.', 'finity-ai'); ?></p>
            </div>
            
            <!-- Progress Indicator -->
            <div class="finity-wizard-progress">
                <div class="finity-wizard-steps">
                    <div class="finity-wizard-step" data-step="1">
                        <div class="step-number">1</div>
                        <div class="step-label"><?php esc_html_e('API Keys', 'finity-ai'); ?></div>
                    </div>
                    <div class="finity-wizard-step" data-step="2">
                        <div class="step-number">2</div>
                        <div class="step-label"><?php esc_html_e('Platform Sync', 'finity-ai'); ?></div>
                    </div>
                    <div class="finity-wizard-step" data-step="3">
                        <div class="step-number">3</div>
                        <div class="step-label"><?php esc_html_e('Defaults', 'finity-ai'); ?></div>
                    </div>
                    <div class="finity-wizard-step" data-step="4">
                        <div class="step-number">4</div>
                        <div class="step-label"><?php esc_html_e('Complete', 'finity-ai'); ?></div>
                    </div>
                </div>
            </div>
            
            <!-- Wizard Steps -->
            <form id="finity-wizard-form" class="finity-wizard-form">
                <!-- Step 1: API Key & Provider Setup -->
                <div class="finity-wizard-step-content" data-step="1">
                    <h2><?php esc_html_e('Step 1: API Key & Provider Setup', 'finity-ai'); ?></h2>
                    <p class="description"><?php esc_html_e('Configure your AI provider API keys. At least one provider is required.', 'finity-ai'); ?></p>
                    
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="wizard-openai-key"><?php esc_html_e('OpenAI API Key', 'finity-ai'); ?></label>
                            </th>
                            <td>
                                <input 
                                    type="password" 
                                ...
                                <button type="button" class="button test-api-key" data-provider="openai">
                                    <?php esc_html_e('Test', 'finity-ai'); ?>
                                </button>
                                <span class="test-status" data-provider="openai"></span>
                                <p class="description">
                                    <?php esc_html_e('Enter your OpenAI API key. Get one at', 'finity-ai'); ?> 
                        ...
                        <tr>
                            <th scope="row">
                                <label for="wizard-gemini-key"><?php esc_html_e('Gemini API Key', 'finity-ai'); ?></label>
                            </th>
                            <td>
                                <input 
                                    type="password" 
                                ...
                                <p class="description">
                                    <?php esc_html_e('Enter your Google Gemini API key. Get one at', 'finity-ai'); ?> 
                    ...
                    <div class="wizard-step-validation" data-step="1">
                        <p class="error-message" style="display: none; color: #dc3232;">
                            <?php esc_html_e('Please configure at least one API key.', 'finity-ai'); ?>
                        </p>
                    </div>
                </div>
                
                <!-- Step 2: Platform Sync Setup -->
                <div class="finity-wizard-step-content" data-step="2" style="display: none;">
                    <h2><?php esc_html_e('Step 2: Platform Sync Setup', 'finity-ai'); ?></h2>
                    <p class="description"><?php esc_html_e('Connect to the Finity AI platform to sync token usage and access advanced features.', 'finity-ai'); ?></p>
                    
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="wizard-platform-url"><?php esc_html_e('Platform API URL', 'finity-ai'); ?></label>
                            </th>
                            <td>
                                <input 
                                    type="url" 
                                    ...
                                    value="<?php echo isset($settings['platform_api_url']) ? esc_attr($settings['platform_api_url']) : 'https://api.finity.ai'; ?>" 
                                    class="regular-text"
                                    placeholder="https://api.finity.ai"
                                />
                                <p class="description">
                        ...
                        <tr>
                            <th scope="row">
                                <label for="wizard-platform-key"><?php esc_html_e('Platform API Key', 'finity-ai'); ?></label>
                            </th>
                            <td>
                                <input 
                                    type="password" 
                                ...
                                <button type="button" class="button test-platform-key">
                                    <?php esc_html_e('Verify', 'finity-ai'); ?>
                                </button>
                                <span class="test-status" data-provider="platform"></span>
                                <p class="description">
                                    <?php esc_html_e('Get your API key from your Finity AI account settings.', 'finity-ai'); ?>
                                </p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">
                                <label for="wizard-platform-user-id"><?php esc_html_e('Platform User ID', 'finity-ai'); ?></label>
                            </th>
                            <td>
                                <input 
                                    type="text" 
                                ...
                                />
                                <p class="description">
                                    <?php esc_html_e('Your user ID on the platform. Find this in your account settings.', 'finity-ai'); ?>
                                </p>
                            </td>
                        </tr>
                    </table>
                    ...
                    <div class="notice notice-info inline">
                        <p><?php esc_html_e('Platform sync is optional. You can skip this step and configure it later in settings.', 'finity-ai'); ?></p>
                    </div>
                </div>
                
                <!-- Step 3: Generation Defaults -->
                <div class="finity-wizard-step-content" data-step="3" style="display: none;">
                    <h2><?php esc_html_e('Step 3: Generation Defaults', 'finity-ai'); ?></h2>
                    <p class="description"><?php esc_html_e('Set default values for content generation. These can be changed later in settings.', 'finity-ai'); ?></p>
                    
                    <table class="form-table">
                        <tr>
                            <th scope="row">
                                <label for="wizard-default-tone"><?php esc_html_e('Default Tone/Style', 'finity-ai'); ?></label>
                            </th>
                            <td>
                        ...
                        <tr>
                            <th scope="row">
                                <label for="wizard-max-words"><?php esc_html_e('Max Word Count', 'finity-ai'); ?></label>
                            </th>
                            <td>
                                <input 
                                    type="number" 
                ...
                <!-- Step 4: Complete -->
                <div class="finity-wizard-step-content" data-step="4" style="display: none;">
                    <div class="finity-wizard-complete">
                        <div class="complete-icon">✓</div>
                        <h2><?php esc_html_e('Setup Complete!', 'finity-ai'); ?></h2>
                        <p><?php esc_html_e('Your Finity AI SEO Writer plugin is now configured and ready to use.', 'finity-ai'); ?></p>
                        <p>
                            <a href="<?php echo admin_url('options-general.php?page=finity-ai-settings'); ?>" class="button button-primary button-large">
                                <?php esc_html_e('Go to Settings', 'finity-ai'); ?>
                            </a>
                            <a href="<?php echo admin_url('options-general.php?page=finity-ai-settings'); ?>" class="button button-large">
                                <?php esc_html_e('Start Using Plugin', 'finity-ai'); ?>
                            </a>
                        </p>
                    </div>
                </div>
    ...
    public function render_settings_page() {
        ...
        if (isset($_GET['settings-updated'])) {
            add_settings_error(
                'finity_ai_messages',
                'finity_ai_message',
                __('Settings saved successfully!', 'finity-ai'),
                'success'
            );
        }
        
        // Display any settings errors
        settings_errors('finity_ai_messages');
        ?>
        <div class="wrap">
            <h1><?php echo esc_html(get_admin_page_title()); ?></h1>
            
            <p>
                <a href="<?php echo admin_url('options-general.php?page=finity-ai-wizard&reset_wizard=1'); ?>" class="button button-secondary">
                    <?php esc_html_e('Run Setup Wizard Again', 'finity-ai'); ?>
                </a>
            </p>
            ...
            <form action="options.php" method="post">
                ...
                settings_fields('finity_ai_settings_group');
                
                // Output settings sections and their fields
                do_settings_sections('finity-ai-settings');
                
                // Output save settings button
                submit_button(__('Save Settings', 'finity-ai'));
                ?>
            </form>
            
            <hr style="margin: 30px 0;" />
            
            <h2><?php esc_html_e('AI Content Generation', 'finity-ai'); ?></h2>
            <div id="finity-ai-generator">
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <label for="generation-prompt"><?php esc_html_e('Prompt', 'finity-ai'); ?></label>
                        </th>
                        <td>
                            <textarea 
                                ...
                                rows="4" 
                                class="large-text"
                                placeholder="<?php esc_attr_e('Enter your content generation prompt here...', 'finity-ai'); ?>"
                            ></textarea>
                            <p class="description">
                                <?php esc_html_e('Describe the content you want to generate.', 'finity-ai'); ?>
                            </p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row">
                            <label for="generation-provider"><?php esc_html_e('AI Provider', 'finity-ai'); ?></label>
                        </th>
                        <td>
                            <select id="generation-provider" name="generation-provider" class="regular-text">
                                <option value="openai"><?php esc_html_e('OpenAI', 'finity-ai'); ?></option>
                                <option value="gemini"><?php esc_html_e('Gemini', 'finity-ai'); ?></option>
                            </select>
                        </td>
                    </tr>
                </table>
    ...
    public function enqueue_scripts($hook) {
        ...
        if ('settings_page_finity-ai-settings' !== $hook && 'settings_page_finity-ai-wizard' !== $hook) {
            return;
        }
        ...
        if ('settings_page_finity-ai-wizard' === $hook) {
            wp_enqueue_style(
                'finity-ai-wizard',
                FINITY_AI_PLUGIN_URL . 'assets/wizard.css',
                array('finity-ai-admin'),
                FINITY_AI_VERSION
            );
        }
        ...
        wp_enqueue_script(
            'finity-ai-admin',
            FINITY_AI_PLUGIN_URL . 'assets/admin.js',
            array('jquery'),
            FINITY_AI_VERSION,
            true
        );
        ...
        if ('settings_page_finity-ai-wizard' === $hook) {
            wp_enqueue_script(
                'finity-ai-wizard',
                FINITY_AI_PLUGIN_URL . 'assets/wizard.js',
                array('jquery', 'finity-ai-admin'),
                FINITY_AI_VERSION,
                true
            );
        }
        ...
        wp_localize_script('finity-ai-admin', 'finityAI', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'restUrl' => rest_url('finity-ai/v1/'),
            'nonce' => wp_create_nonce('finity_ai_nonce'),
            'restNonce' => wp_create_nonce('wp_rest'),
            'strings' => array(
                'generating' => __('Generating content...', 'finity-ai'),
                'success' => __('Content generated successfully!', 'finity-ai'),
                'error' => __('An error occurred. Please try again.', 'finity-ai'),
                'testing' => __('Testing API key...', 'finity-ai'),
                'keyValid' => __('API key is valid!', 'finity-ai'),
                'keyInvalid' => __('API key is invalid or connection failed.', 'finity-ai'),
            )
        ));
        
        // Localize wizard script if on wizard page
        if ('settings_page_finity-ai-wizard' === $hook) {
            wp_localize_script('finity-ai-wizard', 'finityWizard', array(
                'ajaxUrl' => admin_url('admin-ajax.php'),
                'nonce' => wp_create_nonce('finity_ai_wizard_nonce'),
                'strings' => array(
                    'next' => __('Next', 'finity-ai'),
                    'previous' => __('Previous', 'finity-ai'),
                    'skip' => __('Skip', 'finity-ai'),
                    'save' => __('Save & Continue', 'finity-ai'),
                    'testing' => __('Testing...', 'finity-ai'),
                    'verifying' => __('Verifying...', 'finity-ai'),
                    'valid' => __('Valid!', 'finity-ai'),
                    'invalid' => __('Invalid', 'finity-ai'),
                    'required' => __('This field is required', 'finity-ai'),
                    'completing' => __('Completing setup...', 'finity-ai'),
                )
            ));
        }
    }
    ...
    public function register_rest_routes() {
        ...
        register_rest_route('finity-ai/v1', '/generate', array(
            'methods' => 'POST',
        ...
        register_rest_route('finity-ai/v1', '/test-key', array(
            'methods' => 'POST',
    ...
    public function rest_generate_content($request) {
        ...
        if (empty($api_key)) {
            return new WP_Error(
                'missing_api_key',
                sprintf(__('API key for %s is not configured.', 'finity-ai'), ucfirst($provider)),
                array('status' => 400)
            );
        }
    ...
    public function ajax_generate_content() {
        // Verify nonce
        check_ajax_referer('finity_ai_nonce', 'nonce');
        
        // Check permissions
        if (!current_user_can('edit_posts')) {
            wp_send_json_error(array('message' => __('Insufficient permissions.', 'finity-ai')));
        }
        ...
        if (empty($prompt)) {
            wp_send_json_error(array('message' => __('Prompt is required.', 'finity-ai')));
        }
        ...
        if (empty($api_key)) {
            wp_send_json_error(array('message' => sprintf(__('API key for %s is not configured.', 'finity-ai'), ucfirst($provider))));
        }
    ...
    private function generate_ai_content($prompt, $provider, $api_key, $tone, $max_words) {
        ...
        return new WP_Error('invalid_provider', __('Invalid AI provider specified.', 'finity-ai'));
    }
    ...
    private function generate_openai_content($prompt, $api_key) {
        ...
        if (!isset($body['choices'][0]['message']['content'])) {
            return new WP_Error('api_error', __('Unexpected API response format.', 'finity-ai'));
        }
    ...
    public function rest_test_api_key($request) {
        ...
        return rest_ensure_response(array(
            'success' => true,
            'valid' => true,
            'message' => __('API key is valid.', 'finity-ai'),
        ));
    }
    ...
    public function ajax_test_api_key() {
        ...
        if (empty($provider) || empty($api_key)) {
            wp_send_json_error(array('message' => __('Provider and API key are required.', 'finity-ai')));
        }
        ...
        wp_send_json_success(array('message' => __('API key is valid!', 'finity-ai')));
    }
    ...
    private function test_api_key($provider, $api_key) {
        if ($provider === 'openai') {
            ...
            if ($code === 401 || $code === 403) {
                return new WP_Error('invalid_key', __('Invalid API key.', 'finity-ai'));
            }
        ...
        return new WP_Error('invalid_provider', __('Invalid provider specified.', 'finity-ai'));
    }
    ...
    private function sync_token_usage_to_platform($action, $tokens_used, $provider = null, $metadata = array()) {
        ...
        $platform_api_url = isset($settings['platform_api_url']) ? trim($settings['platform_api_url']) : 'https://api.finity.ai';
        ...
        if (!filter_var($api_url, FILTER_VALIDATE_URL)) {
            error_log('Finity AI: Invalid platform API URL: ' . $api_url);
            return;
        }
    ...
    private function send_token_usage_request($api_url, $api_key, $payload, $retry_count = 0) {
        ...
        if (is_wp_error($response)) {
            ...
            if ($retry_count < $max_retries) {
                ...
                wp_schedule_single_event(
                    time() + $delay,
                    'finity_ai_retry_token_sync',
                    array($api_url, $api_key, $payload, $retry_count + 1)
                );
                
                error_log(sprintf(
                    'Finity AI: Token sync failed (attempt %d/%d), retrying in %d seconds. Error: %s',
                    $retry_count + 1,
                    $max_retries,
                    $delay,
                    $error_message
                ...
                error_log(sprintf(
                    'Finity AI: Token sync failed after %d attempts. Final error: %s',
                    $max_retries,
                    $error_message
                ));
            }
        ...
        if ($response_code >= 200 && $response_code < 300) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Finity AI: Token usage synced successfully. Tokens: ' . $payload['tokensUsed']);
            }
            return;
        }
        ...
        if ($response_code >= 400 && $response_code < 500 && $response_code !== 429) {
            error_log(sprintf(
                'Finity AI: Token sync failed with client error (%d): %s',
                $response_code,
                $error_message
            ));
            return;
        ...
        if (($response_code >= 500 || $response_code === 429) && $retry_count < $max_retries) {
            ...
            error_log(sprintf(
                'Finity AI: Token sync failed with server error (%d), retrying in %d seconds. Error: %s',
                $response_code,
                $delay,
                $error_message
            ));
            ...
            error_log(sprintf(
                'Finity AI: Token sync failed after %d attempts. Status: %d, Error: %s',
                $retry_count + 1,
                $response_code,
                $error_message
            ));
    ...
    public function ajax_wizard_save_step() {
        // Verify nonce
        check_ajax_referer('finity_ai_wizard_nonce', 'nonce');
        ...
        if ($step < 1 || $step > 3) {
            wp_send_json_error(array('message' => __('Invalid step.', 'finity-ai')));
        }
        ...
        // Save settings
        update_option('finity_ai_settings', $settings);
        
        wp_send_json_success(array('message' => __('Settings saved successfully.', 'finity-ai')));
    }
    ...
    public function ajax_wizard_test_platform() {
        ...
        if (empty($platform_url) || empty($platform_key) || empty($user_id)) {
            wp_send_json_error(array('message' => __('Platform URL, API key, and User ID are required.', 'finity-ai')));
        }
        ...
        if ($code === 200) {
            wp_send_json_success(array('message' => __('Platform connection verified successfully!', 'finity-ai')));
        } elseif ($code === 401 || $code === 403) {
            wp_send_json_error(array('message' => __('Invalid API key or unauthorized access.', 'finity-ai')));
        } else {
            $body = json_decode(wp_remote_retrieve_body($response), true);
            $error_message = isset($body['error']['message']) ? $body['error']['message'] : __('Connection failed.', 'finity-ai');
    ...
    public function ajax_wizard_complete() {
        ...
        update_option('finity_ai_wizard_completed', true);
        
        wp_send_json_success(array(
            'message' => __('Setup completed successfully!', 'finity-ai'),
            'redirect_url' => admin_url('options-general.php?page=finity-ai-settings')
        ));
    }
    ...
    public function ajax_wizard_reset() {
        ...
        delete_option('finity_ai_wizard_completed');
        
        wp_send_json_success(array(
            'message' => __('Wizard reset successfully.', 'finity-ai'),
            'redirect_url' => admin_url('options-general.php?page=finity-ai-wizard')
        ));
    }
...
register_activation_hook(__FILE__, function() {
    ...
    $default_settings = array(
        ...
        'max_word_count' => 1000,
        'platform_api_url' => 'https://api.finity.ai',
    );
    
    add_option('finity_ai_settings', $default_settings);
    ...
    set_transient('finity_ai_activation_redirect', true, 30);
});
